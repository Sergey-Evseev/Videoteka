...
void printMovie(Movie m) {
	cout << "Name: " << m.name << endl << "Director: " << m.director << endl << "Genre: " << printGenre(m.genre) << endl 
		<< "Rating: " << m.rate << endl << "Price: $" <<  m.price << "\n\n";	
}

void printAllMovies(Movie* movies, int &size) { //передаем массив объектов-структур и его размер
	for (int i = 0; i < size; i++) {
		printMovie(movies[i]);
	}
	cout << endl;
...
int main()
{
	...
	cin >> size;*/
	Movie* films = new Movie[size]{ f1,f2,f3 };//выделяем память под динамический массив типа Movie и передаем туда структуры 
	printAllMovies(films, size);
	films = addMovie(films, size);
	cout << "Add films" << endl;
	printAllMovies(films, size);
